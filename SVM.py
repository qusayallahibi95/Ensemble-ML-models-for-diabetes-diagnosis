...
SVC = SVC(C=1.0, kernel='rbf', degree=5, gamma= 'scale')
# evaluate model
scores = cross_val_score(SVC, data_sample, data_target_sample, scoring='accuracy', cv=kfold, n_jobs=-1)
# report performance
print('Accuracy of SVM = %.3f' % (mean(scores*100)))

results = model_selection.cross_val_predict(estimator=SVC,
                                          X=data_sample,
...
plt.title('Confusion matrix of SVM Technique')
