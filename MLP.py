...
MLP = MLPClassifier(hidden_layer_sizes=300, activation='relu', solver= 'lbfgs',alpha=0.1, learning_rate='adaptive',batch_size=min(200, 600),max_iter=10000, early_stopping= True)
# evaluate model
scores = cross_val_score(MLP, data_sample, data_target_sample, scoring='accuracy', cv=kfold, n_jobs=-1)
# report performance
print('Accuracy of MLP = %.3f' % (mean(scores*100)))

results = model_selection.cross_val_predict(estimator=MLP,
                                          X=data_sample,
...
plt.title('Confusion matrix of MLP Technique')
