...
# evaluate model
scores = cross_val_score(LR, data_sample, data_target_sample, scoring='accuracy', cv=kfold, n_jobs=-1)
# report performance
print('Accuracy of LR = %.3f' % (mean(scores*100)))

results = model_selection.cross_val_predict(estimator=LR,
                                          X=data_sample,
...
plt.title('Confusion matrix of LR Technique')
