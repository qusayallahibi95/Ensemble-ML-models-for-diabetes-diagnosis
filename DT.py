...
# 1-DecisionTreeClassifier       model 1
DT = DecisionTreeClassifier(criterion='entropy', splitter='best', max_depth=8, max_features = 'auto')
# evaluate model
scores = cross_val_score(DT, data_sample, data_target_sample, scoring='accuracy', cv=kfold, n_jobs=-1)
# report performance
print('Accuracy of DT = %.3f' % (mean(scores*100)))

results = model_selection.cross_val_predict(estimator=DT,
                                          X=data_sample,
...
plt.title('Confusion matrix of DT Technique')
