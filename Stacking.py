...
level0 = list()
level0.append(('SVC', SVC(C=1.0, kernel='rbf', degree=5)))
level0.append(('RF', RandomForestClassifier(n_estimators=150, criterion='gini', max_depth=11, min_samples_split=2, min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_features='log2')))
level0.append(('KNN', KNeighborsClassifier(n_neighbors =4, weights= 'distance', algorithm= 'ball_tree')))
level0.append(('LR', LogisticRegression(penalty= 'none', solver= 'lbfgs',max_iter= 10000)))
level0.append(('DT', DecisionTreeClassifier(criterion='entropy', splitter='best', max_depth=8)))
level0.append(('MLP', MLPClassifier(hidden_layer_sizes=300, activation='relu', solver= 'lbfgs',alpha=0.1, learning_rate='adaptive',batch_size=min(200, 600),max_iter=10000)))
...
results = model_selection.cross_val_predict(estimator=model,
                                          X=data_sample,
...
plt.title('Confusion Matrix of Stacked RF+SVM Model')
